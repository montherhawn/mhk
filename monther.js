...
allLinks.forEach((item,index)=>{
    if (currentURL == allLinks[index].href) {
        allLinks[index].classList.add("active");
    }
}
);

const darkModeToggleBtn = document.querySelector('.theme-btn');
...
mql.addEventListener("change", (e)=>(e.matches ? enableDarkMode() : disableDarkMode()));
                ...
                for (var o in r)
                    if (r.hasOwnProperty(o) && r[o]) {
                        ...
                        for (var u = 0; u < s.length; ++u) {
                            ...
                            for (var v = c.pattern || c, m = a.next, y = i; m !== t.tail && !(l && y >= l.reach); y += m.value.length,
                            m = m.next) {
                                var b = m.value;
